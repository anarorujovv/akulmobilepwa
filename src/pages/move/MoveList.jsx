...
const MoveList = ({ route, navigation }) => {
    let theme = useTheme();
    ...
    const fetchingDocumentData = useCallback(async () => {
        ...
        try {
            const element = await api('moves/get.php', obj);
            if (element != null) {
    ...
    const handleDelete = async (id) => {
        if (permission_ver(permissions, 'move', 'D')) {
            await api('moves/del.php?id=' + id, {
    ...
    const renderItem = ({ item, index }) => (
        <>
            <ListItem
                index={index + 1}
                onLongPress={() => {
                    prompt('Yerdəyişmə silməyə əminsiniz?', () => {
                ...
                onPress={() => {
                    if (permission_ver(permissions, 'move', 'R')) {
                        navigation.navigate('move-manage', {
                            id: item.Id
                        })
                    } else {
