...
const ProductCard = ({ navigation, setHasUnsavedChanges }) => {
    ...
    const { document, setDocument, units, setUnits } = useContext(InventoryGlobalContext)
...

                {
                    document.Positions.map((item, index) => (
                        <ListItem
                            ...
                            onPress={() => {
                                navigation.navigate('product-position', { product: item, state: document, setState: setDocument, type: 0, units: units, setUnits: setUnits, setHasUnsavedChanges: setHasUnsavedChanges })
